...
## 7. Locked rule-to-source traceability

The executable fragment used for the full experiment is constrained by `artifact/method/rule_source_ledger.csv`. Each semantic rule used by the oracle is linked to explicit public evidence claims in `artifact/data/corpus_claims.csv`. Rules outside that ledger are treated as out-of-scope rather than silently approximated. A fixture is admissible only if its manifest row lists at least one locked claim id and its canonical SHA-256 hash appears in `artifact/data/fixture_manifest.csv`.
